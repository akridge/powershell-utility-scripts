...
# ------------------------------------------------------------------------------------------------------------------------------------------
#Simple Intro. Lines
Write-Host "                    ****Find and Move Script v. 2.0****                " -foregroundcolor yellow -backgroundcolor DarkCyan
...
foreach ($file in $filenames) {
     ...
     $fullname = Get-ChildItem $searchdir –Recurse | Where-Object {$_.PSIsContainer -eq $False -and ($_.Name) -eq $file} | ForEach-Object {$_.FullName}
...
# ------------------------------------------------------------------------------------------------------------------------------------------
Write-Host "* Starting Move Now...                                               " -foregroundcolor yellow -backgroundcolor DarkCyan
Write-Host "Current Move Directory: $destdir" -foregroundcolor yellow -backgroundcolor DarkCyan
...
foreach ( $file in $filenames ) {
Write-Host "* Moving..." -foregroundcolor yellow -backgroundcolor DarkCyan

    #copies each file to destination. 
    Move-Item $file $destdir
    
    # Logs success/fail of each move 
    echo "$(Get-Date -f o) $? $file" >> "log_files\ $(get-date -f MMddyyyy_hhmm)_movelog.txt"
}
Write-Host "* Move Finished." -foregroundcolor yellow -backgroundcolor DarkCyan
